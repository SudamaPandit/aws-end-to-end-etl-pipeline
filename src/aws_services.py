...
def wait_for_glue_job(job_name: str, run_id: str, poll_seconds: int = 15) -> None:
    glue = boto3.client("glue", region_name=os.getenv("AWS_REGION"))
    while True:
        state = glue.get_job_run(JobName=job_name, RunId=run_id, PredecessorsIncluded=False)["JobRun"]["JobRunState"]
        if state == "SUCCEEDED":
            return
